...
func TestJsonSender_New(t *testing.T) {
	type args struct {
...
func TestJsonSender_WithErrorOperational(t *testing.T) {
	type args struct {
	...
	testCases := []struct {
	...
	}{
		{
			...
			want: []byte(fmt.Sprintf("{\"error\":{\"status\":409,\"error\":\"ALREADY_EXISTS\",\"description\":\"email already exists\"}}\n")),
		},
		{
			...
			want: []byte(fmt.Sprintf("{\"error\":{\"status\":500,\"error\":\"INTERNAL\",\"description\":\"Something went wrong...\"}}\n")),
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
